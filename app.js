...
function applyMarket(market) {
  MARKET          = market;
  const cfg       = MARKET_CONFIG[market] || MARKET_CONFIG.UAE;
...
async function loadFromSupabase() {
  try {
    ...
    state.employees = (emps || []).map(emp => {
      ...
      const allTypes = Object.keys(Object.assign({}, MARKET_CONFIG.UAE.certificates, MARKET_CONFIG.Qatar.certificates));
      allTypes.forEach(t => {
...
async function uploadFileToStorage(empCode, type, file) {
  ...
  const filePath = `${MARKET}/${empCode}/${type}/${Date.now()}.${ext}`;
...
function toggleBulkSelect(empId, checked, el) {
  checked ? bulkSelectedEmpIds.add(empId) : bulkSelectedEmpIds.delete(empId);
  if (el && el.closest) { const tr = el.closest("tr"); if (tr) tr.classList.toggle("bulk-selected-row", checked); }
...
function initSection(type) {
    ...
    try {
      ...
      for (const sibling of group) {
        ...
        if (!certData[sibling] || (!certData[sibling].issueDate && !certData[sibling].expiryDate)) {
          ...
          const inState2 = state.employees.find(x => x.id === id);
          if (inState2) inState2.certificates[sibling] = emptyCert;
          const sibId = await upsertCertificate(dbId, sibling, emptyCert, emp._certIds?.[sibling]);
          if (inState2) { if (!inState2._certIds) inState2._certIds = {}; inState2._certIds[sibling] = sibId; }
        }
      }
      setSyncState("idle");
  ...
  cancelEditBtn.addEventListener("click", () => { resetEmployeeForm(type); setEmployeeFormOpen(type, false); });

  // One green action per section: "+ Add employee" opens the quick form
  const headingActions = employeeForm.closest(".panel")?.querySelector(".panel-heading-actions");
  if (headingActions && !document.getElementById(`showAddEmployee${sfx}`)) {
    const addBtn = document.createElement("button");
    addBtn.id = `showAddEmployee${sfx}`;
    addBtn.type = "button";
    addBtn.className = "primary-btn";
    addBtn.textContent = "+ Add employee";
    headingActions.insertBefore(addBtn, headingActions.firstChild);
    addBtn.addEventListener("click", () => setEmployeeFormOpen(type, employeeForm.classList.contains("hidden")));
  }
  ...
  showBulkCertBtn.addEventListener("click", () => {
    ...
    if (!h) setEmployeeFormOpen(type, false);
    bulkEmpSection.classList.add("hidden"); showBulkEmpBtn.textContent = "Import CSV";
    showBulkCertBtn.textContent = h ? "Attach files" : "Close";
  });
  // Bulk emp toggle
  showBulkEmpBtn.addEventListener("click", () => {
    const h = bulkEmpSection.classList.toggle("hidden");
    if (!h) setEmployeeFormOpen(type, false);
    bulkCertSection.classList.add("hidden"); showBulkCertBtn.textContent = "Attach files";
    showBulkEmpBtn.textContent = h ? "Import CSV" : "✕ Close";
  });
...
function resetEmployeeForm(type) {
  ...
  document.getElementById(`employeeFormTitle${sfx}`).textContent    = "Employees";
...
function editEmployee(id, type) {
  ...
  showView(type); setEmployeeFormOpen(type, true); form.elements.name.focus();
...
function renderStatusChips(type, sfx, baseEmps, active) {
  ...
  if (!select) return;
  select.classList.add("status-select-hidden");
  let row = document.getElementById(`statusChips${sfx}`);
  if (!row) {
    row = document.createElement("div");
    row.id = `statusChips${sfx}`;
    row.className = "chip-row status-chips";
    const heading = select.closest(".panel-heading");
  ...
  const sums = baseEmps.map(e => getCertSummary(e, type));
  ...
  row.innerHTML = chips.map(([val, label]) => {
    const c = count(val);
    const tone = val === "Expired" && c ? " chip-alert" : "";
    return `<button type="button" class="chip${val === active ? " active" : ""}${tone}" data-stat="${val}">${label}<span class="chip-count">${c}</span></button>`;
  }).join("");
}
...
document.addEventListener("click", ev => {
  const tile = ev.target.closest(".cert-overview-panel .cert-stat");
  if (!tile) return;
  const panel = tile.closest(".cert-overview-panel");
  const type  = panel && panel.id ? panel.id.replace("Overview", "") : "";
  if (!CERT_TYPES.includes(type)) return;
  const map = { good: "Valid", watch: "Expiring", warn: "Expiring", bad: "Expired", neutral: "Missing", sched: "Scheduled" };
  const cls = Object.keys(map).find(k => tile.classList.contains(k));
  if (cls) jumpToSection(type, map[cls]);
});

// ── Add employee form lives behind one green button ──────────────────────────
function setEmployeeFormOpen(type, open) {
  const sfx  = SECTION_SUFFIX[type] || Object.assign({}, MARKET_CONFIG.UAE.sectionSuffix, MARKET_CONFIG.Qatar.sectionSuffix)[type];
  const form = document.getElementById(`employeeForm${sfx}`);
  const btn  = document.getElementById(`showAddEmployee${sfx}`);
  if (!form) return;
  form.classList.toggle("hidden", !open);
  if (btn) { btn.textContent = open ? "Close" : "+ Add employee"; btn.className = open ? "secondary-btn" : "primary-btn"; }
  if (open) {
    document.getElementById(`bulkCertSection${sfx}`)?.classList.add("hidden");
    document.getElementById(`bulkEmpSection${sfx}`)?.classList.add("hidden");
  }
}
...
function renderSectionRows(type) {
  ...
  const emps = state.employees.filter(e => {
    ...
    return [e.name,e.employeeId,e.department].join(" ").toLowerCase().includes(q)
      && (dept==="all"||e.department===dept)
      && matchesStatus(s, stat);
  });
  // Everyone in this section who matches search + department (status chips count from here)
  const baseEmps = state.employees.filter(e => certApplies(e,type)
    && [e.name,e.employeeId,e.department].join(" ").toLowerCase().includes(q)
    && (dept==="all"||e.department===dept));
  const sectionTotal = state.employees.filter(e => certApplies(e,type)).length;
  const emptyMsg = sectionTotal === 0
    ? `Nobody on the ${CERTIFICATES[type].label} list yet. Tap <strong>+ Add employee</strong> or import a CSV to get going.`
    : "No matches here. Try another filter or clear the search.";
  renderStatusChips(type, sfx, baseEmps, stat);
  ...
  setRows(`staffRows${sfx}`, emps.map(e=>{
    ...
    const checkCell = canSchedule()
      ? `<td class="bulk-check-cell"><input type="checkbox" class="bulk-checkbox" ${checked} onchange="toggleBulkSelect('${e.id}',this.checked,this)" aria-label="Select ${escHtml(e.name)}"></td>`
      : `<td class="bulk-check-cell"></td>`;
    ...
    const scheduleBtn = canSchedule() && s.status !== "Valid"
      ? `<button class="schedule-btn ${s.status==="Scheduled"?"scheduled-active":""}" title="${s.status==="Scheduled"?`Scheduled: ${fmtDate(s.scheduledDate)}`:"Schedule renewal"}" data-action="schedule-cert" data-eid="${e.id}" data-type="${type}">${s.status==="Scheduled"?"Reschedule":"📅 Schedule"}</button>` : "";
    ...
    return `<tr class="${bulkSelectedEmpIds.has(e.id) ? "bulk-selected-row" : ""}">
      ...
      <td class="date-cell">${fmtDate(s.issueDate)}</td><td class="expiry-cell">${fmtDate(s.expiryDate)}${expiryHint(s.expiryDate)}</td>
      <td class="cert-file-cell">
        ${uploadBtn ? uploadBtn : fileLink(s.record.file)}
...
function badge(status, scheduledDate=""){
  if(status==="Scheduled") return`<span class="badge scheduled" title="Scheduled for ${fmtDate(scheduledDate)}">📅 ${fmtDate(scheduledDate)}</span>`;
  const cls=status==="Valid"?"good":status==="Expiring in 90 Days"?"watch":status==="Expiring in 30 Days"?"warn":status==="Missing"?"neutral":"bad";
  // Plain words people act on; the exact days sit under the expiry date
  const label=status==="Expiring in 30 Days"?"Renew now":status==="Expiring in 90 Days"?"Renew soon":status;
  return`<span class="badge ${cls}" title="${escHtml(status)}">${escHtml(label)}</span>`;
}
const _MON=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
function fmtDate(v){if(!v)return"--";const d=new Date(`${v}T00:00:00`);if(isNaN(d))return"--";return String(d.getDate()).padStart(2,"0")+" "+_MON[d.getMonth()]+" "+d.getFullYear();}
// Short, human hint under an expiry date: "in 3 months", "in 12 days", "5 days overdue"
function expiryHint(v){
  if(!v) return "";
  const d = daysUntil(v);
  let txt, cls;
  if (d < 0)        { txt = Math.abs(d) + (Math.abs(d)===1?" day":" days") + " overdue"; cls = "hint-bad"; }
  else if (d === 0) { txt = "expires today"; cls = "hint-bad"; }
  else if (d <= 30) { txt = "in " + d + (d===1?" day":" days"); cls = "hint-warn"; }
  else if (d <= 90) { txt = "in " + d + " days"; cls = "hint-watch"; }
  else              { const m = Math.round(d/30); txt = "in " + m + (m===1?" month":" months"); cls = "hint-ok"; }
  return `<small class="days-hint ${cls}">${txt}</small>`;
}
// One place that decides if a cert summary matches a status filter value
function matchesStatus(s, stat){
